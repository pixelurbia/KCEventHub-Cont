...
function aiosc_get_page_ticket_preview($ticket, $edit_mode = false, $frontend = false) {
    ...
    if($frontend && $aiosc_settings->get('pages_frontend_enable')) $page = $aiosc_settings->get('page_ticket_preview');
    else $page = 0;
    $admin_page = aiosc_url_apply_query(get_admin_url(null,"/admin.php"),
        array('page'=>'aiosc-ticket-preview','ticket_id'=>$ticket));
    ...
    if(!$frontend || ($frontend && $page < 1) || $edit_mode) {
        ...
        if($page !== false) {
            return aiosc_url_apply_query($page,array('ticket_id'=>$ticket));
        }
        else return $admin_page;
    }
}
function aiosc_get_page_ticket_form($frontend=false, $additional_query=array()) {
    global $aiosc_settings;
    if($frontend && $aiosc_settings->get('pages_frontend_enable')) $page = $aiosc_settings->get('page_ticket_form');
    else $page = 0;
    ...
    if(!$frontend || ($frontend && $page < 1)) return $admin_page;
    else {
        $page = get_permalink($page);
        return $page === false?$admin_page:aiosc_url_apply_query($page,$additional_query);
    }
}
function aiosc_get_page_ticket_list($frontend=false, $additional_query=array()) {
    global $aiosc_settings;
    if($frontend && $aiosc_settings->get('pages_frontend_enable')) $page = $aiosc_settings->get('page_ticket_list');
    else $page = '';
    $admin_page = aiosc_url_apply_query(get_admin_url(null,"/admin.php"),array('page'=>'aiosc-list'));
    $admin_page = aiosc_url_apply_query($admin_page,$additional_query);
    if(empty($page) || is_admin()) return $admin_page;
    else {
        $page = get_permalink($page);
        return $page === false?$admin_page:aiosc_url_apply_query($page,$additional_query);
    }
