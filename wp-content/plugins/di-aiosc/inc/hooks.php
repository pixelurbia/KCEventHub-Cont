...
class aiosc_Hooks {
    ...
    public function user_bulk() {
        ...
        <label class="screen-reader-text" for="aiosc_new_role"><?php esc_html_e( 'Change AIOSC role to&hellip;', 'aiosc' ) ?></label>
    <select name="aiosc_new_role" id="aiosc_new_role" style="max-width: 100%; display: inline-block; float: none;">
            ...
            elseif(
                ...
                if(isset($_POST['frontend'])) {
                    ...
                    if(file_exists($file)) {
                        ...
                        if(!$aiosc_user->can('staff')) $c_query .= " WHERE author_id=$aiosc_user->ID ";
                        $ticket_count = $wpdb->get_var( $c_query . aiosc_get_query($aiosc_user->can('staff')));
                        ...
                        $tickets = aiosc_TicketManager::get_tickets($query . aiosc_get_query($aiosc_user->can('staff')) . $ending_query);
