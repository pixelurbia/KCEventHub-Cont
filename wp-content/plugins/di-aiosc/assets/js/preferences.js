...
function switch_screen(screen_id, additional, li) {
    ...
    response.removeClass('error').removeClass('updated');
    loader.fadeIn(10);
    jQuery.post(ajaxurl, param_data, function(data) {
        aiosc_log(data);
...
function update_departments(action,ids) {
    if(ids.length) {
        if(action == 'delete' || action == 'activate' || action == 'deactivate') {
            ...
            response.removeClass('error').removeClass('updated');
            loader.fadeIn(10);
            jQuery.post(ajaxurl, { action: 'aiosc_departments_update', action2: action, departments: ids}, function(data) {
                aiosc_log(data);
                var res = jQuery.parseJSON(data);
                if(res.result > 0) {
                    jQuery('.aiosc-tab-content').html(res.data.html);
...
function update_priorities(action,ids) {
    if(ids.length) {
        if(action == 'delete' || action == 'activate' || action == 'deactivate') {
            ...
            response.removeClass('error').removeClass('updated');
            loader.fadeIn(10);
            jQuery.post(ajaxurl, { action: 'aiosc_priorities_update', action2: action, priorities: ids}, function(data) {
                aiosc_log(data);
                var res = jQuery.parseJSON(data);
                if(res.result > 0) {
                    jQuery('.aiosc-tab-content').html(res.data.html);
