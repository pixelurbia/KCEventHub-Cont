...
if(!aiosc_is_ticket($ticket) || (!$aiosc_user->can('view_ticket',array('ticket_id'=>$ticket))) || ($edit_mode && !$aiosc_user->can('edit_ticket',array('ticket_id'=>$ticket)))) {
    ...
    if($edit_mode) {
        ...
        $operator_select = '<select name="operator" id="new_operator">';
        foreach($department->get_ops(true) as $obj) {
            $selected = $obj->ID == $operator->ID?'selected="selected"':'';
            $operator_select .= "<option $selected value='$obj->ID'>".$obj->wpUser->display_name."</option>";
        }
        $selected = "";
        $operator_select .= "</select>";

        $author_select = '<select name="author" id="new_author">';
        foreach($available_authors as $obj) {
            $selected = $obj->ID == $author->ID?'selected="selected"':'';
            $author_select .= "<option $selected value='$obj->ID'>".$obj->wpUser->display_name."</option>";
        }
        $selected = "";
    ...
    <script>
        jQuery(document).ready(function($) {
            $(document).on('change','#new_department',function(e) {
                ...
                $.post(ajaxurl, { action: 'aiosc_load_operator_list', department_id: dep_id }, function(data) {
