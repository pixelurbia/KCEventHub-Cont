    ...
    <div class="contentwrap">
      ...
      <h1>Congratulations!</h1>
       <?php while ( have_posts() ) : the_post(); ?>
        <?php 
if ($url == 'http://kcraceday.org') {
  echo '<p>Congratulations! You will now be re-directed to KCRaceday.org to fill out your race permit. Please come back to KCEventhub.org and the My Events tab to submit the other permits needed for your event.</p>';
                ...
                $uniKey = $_GET['uniKey'];
                    ...
                    global $wpdb;
                        ...
                        foreach($entries as $entry){
                            ...
                            $police_traffic = $entry->police_traffic;
                            ...
                            $events[$eventID][$eventName][$entryID]['police_traffic'] = $police_traffic;
                    }
            ...
            foreach ($events as $eID => $eValue){
                ...
                foreach ($eValue as $eName => $eKey){
                    ...
                    foreach ($eKey as $enID => $enData)
                    {
                        ...
                        $police_traffic = $enData['police_traffic'];
                ...
                if ( $id = '2' ) {  //only the main app
                   ...
                   $link = '&eventType='. $eventType .'&uniKey='. $eID .'&eventName='.$enData['event_name'].'&eventDate='.$event_date.'&address='.$enData['address'].'&addresstwo='.$enData['addresstwo'].'&city='.$enData['city'].'&state='.$enData['state'].'&zip='.$enData['zip'].'&first_name='.$enData['first_name'].'&last_name='.$enData['last_name'].'&phone='.$enData['phone'].'&email='.$enData['email'].'&date_end='.$date_end.'&time_start='.$time_start.'&time_end='.$time_end.'&spon_name='.$spon_name.'&spon_address='.$spon_address.'&spon_address_two='.$spon_address_two.'&spon_city='.$spon_city.'&spon_state='.$spon_state.'&spon_zip='.$spon_zip.'&spon_phone='.$spon_phone.'&spon_email='.$spon_email.'&day_phone='.$day_phone.'&day_contact='.$day_contact.'&location_des='.$location_des.'&location='.$location.'&police_traffic='.$police_traffic.'';
                        ...
                        if ( $ems_permit  >= '1000') { $output .= '<li>
                            KC Fire Dept EMS<br>
                        <a href="'. $homelink .'/wp-content/uploads/2015/07/Fire-EMS-Guidelines1.pdf">Instructions</a> | Dealine: 60 days prior <br>Your event may require EMS presence. Contact Brian Trickey at KCFD for assistance at <a href="mailto:KEVIN.TRICKEY@KCMO.ORG">KEVIN.TRICKEY@KCMO.ORG</a>.
                        <br>
                        <a href="'. $homelink .'/fire-ems/?child=Yes'.$link.'&peopleCount='.$ems_permit.'&overOne=Yes">Review and Submit for Fire EMS Planning</a><</li>'; }
                        ...
                        <br>
                        <a href="'. $homelink .'/fire-tents-application/?child=Yes'.$link.'&gas='.$fire_permit.'&rides='.$fire_permit.'&firework='.$fire_permit.'">Review & Submit for fire tent and canopies</a></li>'; }
                        if ( $traffic_permit  == 'Yes') { $output .= '<li>
                            Traffic Control <br>
                            Deadline: 30 days prior <br>
                            <a href="'. $homelink .'/traffic-control/?child=Yes'.$link.'">Fill Out The Traffic Control Permit</a><br>
                            <br>Your event may require Traffic Officer presence. Contact Kevin Gooch at KCPD Traffic division for assistance 816-329-0911.
                            </li>'; }
                        if ( $police_traffic  == 'Yes') { $output .= '<li class="child '.$eID.'formID26"> 
                            Police Traffic <br>
                            Deadline: 30 days prior <br>
                        ...
                        <br>
                        <a href="'. $homelink .'/police-security-permit/?child=Yes'.$link.'&admission='.$admission.'&alcohol='.$alcServ.'&dance='.$dPermit.'&noise='.$nPermit.'">Review & Submit for KCPD Security Planning</a></li>'; }
