...
class Calendar {
	...
	function load_month_events() {
		...
		foreach ($query as $post) {
			...
			// 	$showtimes = get_post_meta( $post->ID, 'show_times', true);
//faild attempt at portning over ACF repeater fields
			// 	if( $showtimes ) {
			// 	  for( $i = 0; $i < $showtimes; $i++ ) {
			// 	    $days =  get_post_meta( $post->ID, 'show_times_' . $i . '_show_date', true ) ; 
			// 	    $times =  get_post_meta( $post->ID, 'show_times_' . $i . '_show_time', true ) ;
			// 	    $links =  get_post_meta( $post->ID, 'show_times_' . $i . '_link_to_tickets', true );
			// 	  }

			// }

			// $event['showinfo'] = $showtimes;
			// $event['day'] = $days;
			// $event['link'] = $links;
	...
	function load_list_events() {
		...
		foreach ($query as $post) {
			...
			$event['content'] = substr($excerpt, 0, 155);
			$this->events[] = $event;
		}
		$this->duplicate_multiday();
		usort($this->events, "cmp");
	...
	function events_after_today_until($date) {
		...
		return $wpdb->get_results( 
		  ...
		  SELECT *
			...
			WHERE wp_posts.post_type = 'shows'
